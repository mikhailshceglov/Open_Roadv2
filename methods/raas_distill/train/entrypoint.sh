...
# The stages are modules of the installed package now, not loose scripts, so
# they are invoked with -m from the repository root rather than by path.
TRAIN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${TRAIN_DIR}/../../.." && pwd)"
STAGE_MODULE="methods.raas_distill.train.stages"
cd "$REPO_ROOT"
...
stage_inputs() {
  ...
  python3 -m methods.raas_distill.train.s3sync get data/raas-data.tar.gz /tmp/raas-data.tar.gz
  ...
  python3 -m methods.raas_distill.train.s3sync get data/ckpt/model_final_17c1ee.pkl \
...
publish_outputs() {
  ...
  for directory in checkpoints eval export; do
    python3 -m methods.raas_distill.train.s3sync put-dir "${OUT_ROOT}/${directory}" "out/${directory}" || true
  done
  python3 -m methods.raas_distill.train.s3sync put "${OUT_ROOT}/summary.json" out/summary.json || true
}
...
run_stage label  "${STAGE_MODULE}.label"
run_stage train  "${STAGE_MODULE}.train"
run_stage eval   "${STAGE_MODULE}.evaluate"
run_stage export "${STAGE_MODULE}.export"
