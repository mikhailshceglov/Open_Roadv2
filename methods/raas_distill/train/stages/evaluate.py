...
from ...student.preprocess import IMAGENET_MEAN, IMAGENET_STD, SIZE_MULTIPLE
from ..common import _scripts_module, data_root, list_corpus, out_root, target_path
...
class StudentPredictor:
    ...
    def __init__(self, checkpoint: Path, device, short_side: int = 1024):
        ...
        # pretrained=False: the checkpoint replaces every weight, so fetching
        # the backbone first only costs a download.
        self.model = Student(pretrained=False).to(device).eval()
    ...
    def get_soft_mask(self, image_bgr):
        ...
        #
        # This deliberately differs from student.preprocess.to_tensor, which
        # scales in both directions -- that is the path the published
        # short-side table was measured on. Do not "unify" the two without
        # re-measuring: they are different operating points, not a bug.
